...
// Helper to make local fetch requests to Next.js server or test DB directly
async function runVerification() {
  console.log('=== INICIANDO TESTES DE ARQUITETURA E CONCORRÊNCIA ===\n');

  const { createSession, getSession, autosaveSession, submitSession, getAllSubmissions, getAggregatedSummary } = await import('../src/lib/db.ts');

  // 1. Teste de Garantia de Anonimato
  console.log('1. A testar Garantia Arquitetural de Anonimato...');
  const anonSession = createSession({
    identityMode: 'anonymous',
    name: 'Nome Que Não Deve Ser Gravado',
    contact: '999999999'
  });

  const anonDbRecord = getSession(anonSession.id, anonSession.sessionToken);
  ...
  const identifiedSession = createSession({
    identityMode: 'identified',
    name: 'Catarina Silva',
    contact: '912345678'
  });
  const idRecord = getSession(identifiedSession.id, identifiedSession.sessionToken);
  ...
  const workers = Array.from({ length: 10 }).map((_, i) => ({
    ...
    identityMode: i % 2 === 0 ? 'identified' : 'anonymous',
    answers: {
  ...
  const sessions = await Promise.all(workers.map(w => Promise.resolve(createSession({
  ...
  await Promise.all(sessions.map((s, i) => {
    return Promise.resolve(autosaveSession(s.id, s.sessionToken, 2, workers[i].answers));
  }));

  // Simular submissões finais paralelas
  await Promise.all(sessions.map((s, i) => {
    return Promise.resolve(submitSession(s.id, s.sessionToken));
  }));

  // Verificar isolamento e ausência de contaminação cruzada
  for (let i = 0; i < 10; i++) {
    const record = getSession(sessions[i].id);
  ...
  const postSubmitAutosave = autosaveSession(firstSession.id, firstSession.sessionToken, 3, { 1: { text: 'Tentativa de alteração pós-submissão' } });
